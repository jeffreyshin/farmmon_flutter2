...
//response = requests.get(api_url, headers=headers)
    ...
    response = await http.get(Uri.parse(rda_30mUrl));
    try {
...
///////////////////////////////////////////////////////////////////////////
    ...
    await pepper.apiRequestPepper(context).then((value) {
      _initExampleData();
      tag2 = 1; //1;
      setState(() {});
    });
    ...
    final items = [
      for (int row = 0; row < rows.length; row++)
        for (int col = 0; col < columns.length; col++)
          HeatmapItem(
              value: ((row == 0)
                  ? (pepperPestL[col].PepperBacterialLeafSpot as double)
                  : (row == 1)
                      ? (pepperPestL[col].PepperBacterialWilt as double)
                      : (row == 2)
                          ? (pepperPestL[col].PepperOrientalTobaccoBudworm
                              as double)
                          : (row == 3)
                              ? (pepperPestL[col].Pepper_Antracnose as double)
                              : pepperPestL[col].WesternFlowerThrips
