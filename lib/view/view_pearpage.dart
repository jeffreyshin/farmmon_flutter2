...
class PearPage extends StatefulWidget {
  ...
  @override
  _PearPageState createState() => _PearPageState();
}
...
///////////////////////////////////////////////////////////////////////////
    ...
    await pear.apiRequestPear(context).then((value) {
      _initExampleData();
      tag2 = 1; //1;
      setState(() {});
    });
    ...
    final items = [
      for (int row = 0; row < rows.length; row++)
        for (int col = 0; col < columns.length; col++)
          HeatmapItem(
              value: ((row == 0)
                  ? (pearPestL[col].PearGaru as double)
                  : (row == 1)
                      ? (pearPestL[col].PearScab as double)
                      : (row == 2)
                          ? (pearPestL[col].AppleWhiteRot as double)
                          : (row == 3)
                              ? (pearPestL[col].AppleGuln as double)
                              : (row == 4)
                                  ? (pearPestL[col].PearBsun as double)
                                  : (row == 5)
                                      ? (pearPestL[col].OrientalFruitMoth
                                          as double)
                                      : (row == 6)
                                          ? (pearPestL[col].PlumFruitMoth
                                              as double)
                                          : (row == 7)
                                              ? (pearPestL[col].AppleBoks
                                                  as double)
                                              : (row == 8)
                                                  ? (pearPestL[col]
