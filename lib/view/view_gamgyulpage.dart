...
class GamgyulPage extends StatefulWidget {
  const GamgyulPage({Key? key}) : super(key: key);

  @override
  State<GamgyulPage> createState() => _GamgyulPageState();
}

class _GamgyulPageState extends State<GamgyulPage> {
  ...
  void _initExampleData() {
    const rows = ['화살깍지벌레', '화살깍지벌레2', '감귤궤양병', '검은점무늬병', '감귤거뎅이병', '볼록총채벌레'];
    const columns = [
      '일',
      '월',
      '화',
    ...
    heatmapDataPower = HeatmapData(
      ...
      radius: 6.0,
      items: items,
      colorPalette: colorPaletteDeepOrange,
    );
  }
