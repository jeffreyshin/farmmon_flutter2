...
class CitrusPage extends StatefulWidget {
  const CitrusPage({Key? key}) : super(key: key);

  @override
  _CitrusPageState createState() => _CitrusPageState();
}

class _CitrusPageState extends State<CitrusPage> {
  String? baseTime;
  String? baseDate;
  String? baseDate_2am;
  String? baseTime_2am;
...
///////////////////////////////////////////////////////////////////////////
    ...
    await citrus.apiRequestCitrus(context).then((value) {
      _initExampleData();
      tag2 = 1; //1;
      setState(() {});
    });
    ...
    final items = [
      for (int row = 0; row < rows.length; row++)
        for (int col = 0; col < columns.length; col++)
          HeatmapItem(
              value: ((row == 0)
                  ? (citrusPestL[col].ArrowHeadScale as double)
                  : (row == 1)
                      ? (citrusPestL[col].ArrowHeadScale2 as double)
                      : (row == 2)
                          ? (citrusPestL[col].CitrusCranker as double)
                          : (row == 3)
                              ? (citrusPestL[col].CitrusMelanose as double)
                              : (row == 4)
                                  ? (citrusPestL[col].CitrusScab2 as double)
                                  : (citrusPestL[col].RiceBlast
