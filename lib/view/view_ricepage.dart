...
///////////////////////////////////////////////////////////////////////////
    ...
    await rice.apiRequestRice(context).then((value) {
      _initExampleData();
      tag2 = 1; //1;
      setState(() {});
    });
    ...
    final items = [
      for (int row = 0; row < rows.length; row++)
        for (int col = 0; col < columns.length; col++)
          HeatmapItem(
              value: ((row == 0)
                  ? (ricePestL[col].PearGaru as double)
                  : (row == 1)
                      ? (ricePestL[col].PearScab as double)
                      : (row == 2)
                          ? (ricePestL[col].AppleWhiteRot as double)
                          : (row == 3)
                              ? (ricePestL[col].AppleGuln as double)
                              : (row == 4)
                                  ? (ricePestL[col].PearBsun as double)
                                  : (row == 5)
                                      ? (ricePestL[col].OrientalFruitMoth
                                          as double)
                                      : (row == 6)
                                          ? (ricePestL[col].PlumFruitMoth
                                              as double)
                                          : (row == 7)
                                              ? (ricePestL[col].AppleBoks
                                                  as double)
                                              : (row == 8)
                                                  ? (ricePestL[col]
